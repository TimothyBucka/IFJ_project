    ...
    while (1) {
        ...
        case STATE_START:
            ...
            if (lexer->c == '$') {
                ...
                lexer->state = STATE_SEMICOLON;
            }

            else if (lexer->c == '(') {
                lexer->state = STATE_PARENTHESIS_LEFT;
            }
            else if (lexer->c == ')') {
                lexer->state = STATE_PARENTHESIS_RIGHT;
            }

            else if (lexer->c == '{') {
                lexer->state = STATE_CURLY_LEFT;
            } else if (lexer->c == '}') {
                lexer->state = STATE_CURLY_RIGHT;
            }
        ...
        case STATE_PARENTHESIS_LEFT:
            printf("Token is left parenthesis\n");
            lexer->state = STATE_START;
            Token->ID = TOKEN_ID_PARENTHESIS_LEFT;
            lexer_advance(lexer);
            return;
        case STATE_PARENTHESIS_RIGHT:
            printf("Token is right parenthesis\n");
            lexer->state = STATE_START;
            Token->ID = TOKEN_ID_PARENTHESIS_RIGHT;
            lexer_advance(lexer);
            return;

        case STATE_CURLY_LEFT:
            printf("Token is left curly bracket\n");
            lexer->state = STATE_START;
            Token->ID = TOKEN_ID_CURLY_LEFT;
            lexer_advance(lexer);
            return;
        case STATE_CURLY_RIGHT:
            printf("Token is right curly bracket\n");
            lexer->state = STATE_START;
            Token->ID = TOKEN_ID_CURLY_RIGHT;
            lexer_advance(lexer);
            return;
